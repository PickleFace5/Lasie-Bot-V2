...
public class LeaveCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        if ((!event.getMember().hasPermission(Permission.VOICE_MOVE_OTHERS) || !(Objects.requireNonNull(guild.getAudioManager().getConnectedChannel()).getMembers().size() <= 2)) && !event.getUser().getId().equals("535622235755380746")) {
