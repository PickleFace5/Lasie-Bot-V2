...
public class LeaveCommand extends ListenerAdapter {
    ...
    public void onSlashCommand(@NotNull SlashCommandEvent event) {
        ...
        if (!memberVoiceState.inVoiceChannel() || memberVoiceState.getChannel() != guild.getAudioManager().getConnectedChannel()) {
        ...
        if (!event.getMember().isOwner() || !(Objects.requireNonNull(guild.getAudioManager().getConnectedChannel()).getMembers().size() <= 2)) {
